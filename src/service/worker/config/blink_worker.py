...
class BlinkWorker(BaseWorker):
    ...
    def thread_execute(self):
        ...
        output_motion, fnos = BlinkUsecase().create_blink(
            ...
            self.panel.replace_set.below_eyebrow_morph_ctrl.GetValue(),
            self.panel.replace_set.blink_morph_ctrl.GetValue(),
    ...
    def output_log(self):
        output_log_path = os.path.join(get_root_dir(), f"{os.path.basename(self.panel.output_motion_ctrl.path)}_blink.log")
        # 出力されたメッセージを全部出力
