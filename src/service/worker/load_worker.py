...
class LoadWorker(BaseWorker):
    ...
    def thread_execute(self):
        ...
        if self.panel.model_ctrl.valid() and not self.panel.model_ctrl.data:
            ...
            digest = self.panel.model_ctrl.reader.read_hash_by_filepath(
                self.panel.model_ctrl.path
            )
            ...
            if original_model:
                ...
                original_model = self.panel.model_ctrl.reader.read_by_filepath(
                    self.panel.model_ctrl.path
                )
        ...
        if self.panel.motion_ctrl.valid() and (
            not self.panel.motion_ctrl.data or is_model_change
        ):
            logger.info("モーション読み込み開始", decoration=MLogger.Decoration.BOX)

            digest = self.panel.motion_ctrl.reader.read_hash_by_filepath(
                self.panel.motion_ctrl.path
            )
            ...
            if original_motion:
                ...
                original_motion = self.panel.motion_ctrl.reader.read_by_filepath(
                    self.panel.motion_ctrl.path
                )
    ...
    def output_log(self):
        output_log_path = os.path.join(
            get_root_dir(),
            f"{os.path.basename(self.panel.output_motion_ctrl.path)}_load.log",
        )
        # 出力されたメッセージを全部出力
