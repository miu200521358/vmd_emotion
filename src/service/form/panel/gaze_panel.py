...
class GazePanel(ServiceCanvasPanel):
    ...
    def _initialize_service_ui(self) -> None:
        ...
        self.gaze_title_ctrl = wx.StaticText(
            self.window,
            wx.ID_ANY,
            __("目線生成パラメーター: "),
            wx.DefaultPosition,
        ...
        self.gaze_infection_ctrl = WheelSpinCtrlDouble(
            self.window, initial=0.4, min=0.1, max=1.0, inc=0.01, size=wx.Size(55, -1)
        )
        ...
        self.gaze_limit_upper_x_ctrl = WheelSpinCtrl(
            self.window, initial=3, min=0, max=45, size=wx.Size(50, -1)
        )
        ...
        # --------------
        gaze_limit_lower_x_tooltip = __(
            "目線キーフレで設定する縦方向の値の下限。\n下限を超えた回転量になった場合、下限までしか動かしません。"
        )

        self.gaze_limit_lower_x_title_ctrl = wx.StaticText(
            self.window, wx.ID_ANY, __("縦下限"), wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        self.gaze_limit_lower_x_ctrl = WheelSpinCtrl(
            self.window, initial=-7, min=-45, max=0, size=wx.Size(50, -1)
        )
        ...
        # --------------
        gaze_limit_upper_y_tooltip = __(
            "目線キーフレで設定する横方向の値の上限（向かって左側）。\n上限を超えた回転量になった場合、上限までしか動かしません。"
        )

        self.gaze_limit_upper_y_title_ctrl = wx.StaticText(
            self.window, wx.ID_ANY, __("横上限"), wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        self.gaze_limit_upper_y_ctrl = WheelSpinCtrl(
            self.window, initial=10, min=0, max=45, size=wx.Size(55, -1)
        )
        ...
        # --------------
        gaze_limit_lower_y_tooltip = __(
            "目線キーフレで設定する横方向の値の下限（向かって右側）。\n下限を超えた回転量になった場合、下限までしか動かしません。"
        )

        self.gaze_limit_lower_y_title_ctrl = wx.StaticText(
            self.window, wx.ID_ANY, __("横下限"), wx.DefaultPosition, wx.DefaultSize, 0
        )
        ...
        self.gaze_limit_lower_y_ctrl = WheelSpinCtrl(
            self.window, initial=-10, min=-45, max=0, size=wx.Size(55, -1)
        )
        ...
        self.gaze_reset_ctrl = WheelSpinCtrl(
            self.window, initial=12, min=5, max=30, size=wx.Size(50, -1)
        )
        ...
        self.gaze_blink_ctrl = WheelSpinCtrlDouble(
            self.window, initial=0.2, min=0.0, max=1.0, inc=0.01, size=wx.Size(55, -1)
        )
