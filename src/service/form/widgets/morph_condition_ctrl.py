...
class MorphConditionCtrl:
    def __init__(
        ...
        # 対象モーフ
        self.morph_name_ctrl = MorphChoiceCtrl(
            self.panel,
            self.window,
            "調整モーフ",
            120,
        ...
        # 置換モーフ
        self.replace_morph_name_ctrl = MorphChoiceCtrl(
            self.panel,
            self.window,
            "置換モーフ",
            120,
            "選択モーフを置換したい場合に選択してください",
        )
        ...
        # 下限値
        self.min_ctrl = WheelSpinCtrlDouble(
            self.window,
            initial=-0.1,
            min=-100.0,
            max=100.0,
        ...
        # 上限値
        self.max_ctrl = WheelSpinCtrlDouble(
            self.window,
            initial=1.1,
            min=-100.0,
            max=100.0,
        ...
        # 開始X
        self.start_x_ctrl = WheelSpinCtrl(
            self.window, initial=70, min=0, max=127, size=wx.Size(60, -1)
        )
        self.start_x_ctrl.SetToolTip(
            __(
                "補間曲線の開始Xの値。補間曲線はプレビュー（目）アイコンから確認・調整できます"
            )
        )
        self.sizer.Add(self.start_x_ctrl, 0, wx.ALL, 3)

        # 開始Y
        self.start_y_ctrl = WheelSpinCtrl(
            self.window, initial=10, min=0, max=127, size=wx.Size(60, -1)
        )
        self.start_y_ctrl.SetToolTip(
            __(
                "補間曲線の開始Yの値。補間曲線はプレビュー（目）アイコンから確認・調整できます"
            )
        )
        self.sizer.Add(self.start_y_ctrl, 0, wx.ALL, 3)

        # 終了X
        self.end_x_ctrl = WheelSpinCtrl(
            self.window, initial=57, min=0, max=127, size=wx.Size(60, -1)
        )
        self.end_x_ctrl.SetToolTip(
            __(
                "補間曲線の終了Xの値。補間曲線はプレビュー（目）アイコンから確認・調整できます"
            )
        )
        self.sizer.Add(self.end_x_ctrl, 0, wx.ALL, 3)

        # 終了Y
        self.end_y_ctrl = WheelSpinCtrl(
            self.window, initial=117, min=0, max=127, size=wx.Size(60, -1)
        )
        self.end_y_ctrl.SetToolTip(
            __(
                "補間曲線の終了Yの値。補間曲線はプレビュー（目）アイコンから確認・調整できます"
            )
        )
        ...
        self.history_ctrl = wx.Button(
            self.window, wx.ID_ANY, __("履歴"), wx.DefaultPosition, wx.Size(80, -1)
        )
        ...
        self.bezier_view_ctrl: ImageButton = ImageButton(
            ...
            self.on_show_bezier,
            __(
                "ボタンをONにすると、補間曲線の形や補間曲線に準拠したモーフの変化をプレビューで確認できます"
            ),
        )
    ...
    def on_show_histories(self, event: wx.Event) -> None:
        ...
        with wx.SingleChoiceDialog(
            ...
            idx = self.replace_morph_name_ctrl.choice_ctrl.FindString(
                history.get("replace_morph_name", "")
            )
            if idx >= 0:
    ...
    def history(self) -> Optional[dict[str, str]]:
        ...
        return {
            ...
            "replace_morph_name": str(
                self.replace_morph_name_ctrl.choice_ctrl.GetStringSelection()
            ),
