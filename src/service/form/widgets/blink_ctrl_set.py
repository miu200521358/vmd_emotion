...
class BlinkCtrlSet:
    def __init__(self, parent: BasePanel, window: wx.ScrolledWindow) -> None:
        ...
        self.condition_choice_ctrl.SetToolTip(
            "\n".join(
                [
                    ...
                    __(
                        "確率が0より大きい場合、該当条件に合致するキーフレでまばたきを生成します。"
                    ),
                ]
            )
        )
        ...
        condition_probability_tooltip = "\n".join(
            [
                __("まばたきを生成する条件の発生確率。"),
                __(
                    "100%の場合、該当する箇所で必ず発生しますが、近隣により上位（順番が上）のまばたきの発生が予定されている場合、そちらを優先します。"
                ),
            ]
        )

        self.condition_probability_title_ctrl = wx.StaticText(
            self.window,
            wx.ID_ANY,
            __("条件発生確率"),
            wx.DefaultPosition,
        ...
        self.condition_probability_ctrl = WheelSpinCtrl(
            ...
            max=100,
            size=wx.Size(60, -1),
            change_event=self.on_change_probability,
        )
        ...
        self.half1_btn_ctrl.SetToolTip(
            __("まばたき生成条件の発生確率を40%に設定します")
        )
        ...
        self.full_btn_ctrl.SetToolTip(
            __("まばたき生成条件の発生確率を100%に設定します")
        )
        ...
        self.linkage_depth_title_ctrl = wx.StaticText(
            self.window,
            wx.ID_ANY,
            __("連動の深さ"),
            wx.DefaultPosition,
        ...
        self.linkage_depth_ctrl = WheelSpinCtrlDouble(
            self.window, initial=0.5, min=0, max=1, inc=0.1, size=wx.Size(60, -1)
        )
        self.linkage_depth_ctrl.SetToolTip(linkage_depth_tooltip)
        self.sizer.Add(self.linkage_depth_ctrl, 0, wx.ALL, 3)

        # --------------
        blink_span_tooltip = __(
            "まばたきの間隔\n値が小さいほど、細かくまばたきをします"
        )

        self.blink_span_title_ctrl = wx.StaticText(
            self.window, wx.ID_ANY, __("間隔"), wx.DefaultPosition, wx.DefaultSize, 0
        )
        self.blink_span_title_ctrl.SetToolTip(blink_span_tooltip)
        self.sizer.Add(self.blink_span_title_ctrl, 0, wx.ALL, 3)

        self.blink_span_ctrl = WheelSpinCtrl(
            self.window, initial=60, min=10, max=150, size=wx.Size(60, -1)
        )
    ...
    def on_change_condition(self, event: wx.Event) -> None:
        ...
        self.condition_probability_ctrl.SetValue(
            self.condition_probabilities[condition_name]
        )
    ...
    def on_change_probability(self, event: wx.Event) -> None:
        ...
        self.condition_probabilities[
            condition_name
        ] = self.condition_probability_ctrl.GetValue()
