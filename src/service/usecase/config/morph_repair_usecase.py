...
class MorphRepairUsecase:
    def repair(
        ...
        morph_fnos = sorted(
            set(
                [
                    ...
                    for mf in motion.morphs[morph_name]
                ]
            )
        )
        ...
        if not morph_fnos:
            logger.warning(
                "モーフによる変形があるキーフレが見つからなかったため、処理を中断します",
        ...
        for morph in model.morphs:
            ...
            for offset in morph.offsets:
                ...
                if model.morphs[group_offset.morph_index].morph_type in [
                    MorphType.VERTEX,
                    MorphType.BONE,
                ]:
        ...
        # モーフの変形量閾値の時の変形を保持
        morph_max_vertices: np.ndarray = np.zeros(
            (len(target_morph_names), len(model.vertices), 3)
        )
        morph_vertex_indexes: dict[str, set[int]] = {}

        for midx, morph_name in enumerate(target_morph_names):
            vertex_indexes, morph_max_vertices[midx] = self.get_vertex_positions(
                model, morph_name, repair_factor
            )
        ...
        for fidx, fno in enumerate(morph_fnos):
            logger.count(
                "モーフ破綻チェック",
                index=fidx,
                total_index_count=len(morph_fnos),
                display_block=100,
            )
            ...
            for i in range(100):
                key_morph_vertex_positions: np.ndarray = np.zeros(
                    (len(target_morph_names), len(model.vertices), 3)
                )
                ...
                for midx, (morph_name, ratio) in enumerate(fno_morph_ratios.items()):
                    if np.isclose(ratio, 0.0):
                        continue
                    morph = model.morphs[morph_name]
                    _, key_morph_vertex_positions[midx] = self.get_vertex_positions(
                        model, morph_name, ratio
                    )

                broken_vertex_indexes = np.unique(
                    np.where(
                        np.logical_and(
                            np.sum(np.abs(key_morph_vertex_positions), axis=0)
                            > max_morph_vertex_positions,
                ...
                max_factor_morph_vertex_positions = max_morph_vertex_positions[
                    broken_vertex_indexes
                ]
                broken_factor_morph_vertex_positions = np.sum(
                    key_morph_vertex_positions[:, broken_vertex_indexes], axis=0
                )
                factor_broken_vertex_indexes = np.unique(
                    np.where(
                        np.logical_and(
                            np.abs(
                                broken_factor_morph_vertex_positions
                                / max_factor_morph_vertex_positions
                            )
                            > repair_factor,
                ...
                for midx, (morph_name, ratio) in enumerate(fno_morph_ratios.items()):
                    if np.isclose(ratio, 0.0):
                        ...
                        continue
                    morph_start_fno, _, morph_end_fno = motion.morphs[
                        morph_name
                    ].range_indexes(fno)
                    # 絶対値で大きい方の変化量を採用する
                    abs_ratios = [
                    ...
                    # 破綻している場合の調整キーフレ
                    key_morph_fnos[morph_name] = (
                        morph_start_fno if morph_flg == 0 else morph_end_fno
                    )
                    # 破綻している頂点とモーフの変形対象頂点の重複INDEX
                    key_morph_broken_vertex_indexes = np.array(
                        list(
                            morph_vertex_indexes[morph_name]
                            & set(
                                broken_vertex_indexes[
                                    factor_broken_vertex_indexes
                                ].tolist()
                            )
                        )
                    )
                    key_morph_broken_vertex_index_counts.append(
                        float(len(key_morph_broken_vertex_indexes))
                    )
                    ...
                    if 0 < len(key_morph_broken_vertex_indexes):
                        key_morph_broken_vertex_positions.append(
                            float(
                                np.sum(
                                    np.abs(
                                        key_morph_vertex_positions[
                                            midx, key_morph_broken_vertex_indexes
                                        ]
                                    )
                                )
                            )
                ...
                max_ratio_morph_name = list(fno_morph_ratios.keys())[
                    np.lexsort(
                        (
                            key_morph_broken_vertex_positions,
                            key_morph_broken_vertex_index_counts,
                        )
                    )[-1]
                ]
                ...
                # 変化量を小さくする
                target_morph_original_ratio = motion.morphs[max_ratio_morph_name][
                    target_fno
                ].ratio
                target_morph_repair_ratio = target_morph_original_ratio - (
                    max(0.05, abs(target_morph_original_ratio) * 0.1)
                    * np.sign(target_morph_original_ratio)
                )
                motion.morphs[max_ratio_morph_name][
                    target_fno
                ].ratio = target_morph_repair_ratio
                # 出力は補正値のみ設定
                rmf = VmdMorphFrame(
                    target_fno, max_ratio_morph_name, target_morph_repair_ratio
                )
                ...
                logger.info(
                    ...
                    m=", ".join(
                        [
                            f"{m}({r:.3f})"
                            for m, r in fno_morph_ratios.items()
    ...
    def get_vertex_positions(
        self, model: PmxModel, morph_name: str, ratio: float
        ...
        if morph.morph_type == MorphType.VERTEX:
            for offset in morph.offsets:
                ...
                morph_vertices[vertex_offset.vertex_index] += (
                    vertex_offset.position * ratio
                ).vector
            ...
            for offset in morph.offsets:
                ...
                for bone_vertex_index in bone_vertices:
                    ...
                    mat = np.zeros((4, 4))
                    for bone_index, bone_weight in zip(
                        vertex.deform.indexes, vertex.deform.weights
                    ):
                        mat += (
                            morph_matrixes[
                    ...
                    # 変形後の頂点位置の差分を保持
                    morph_vertices[bone_vertex_index] += (
                        MMatrix4x4(mat) * MVector3D()
                    ).vector
            ...
            for offset in morph.offsets:
                ...
                if part_morph.morph_type == MorphType.VERTEX:
                    ...
                    motion = VmdMotion()
                    motion.morphs[part_morph.name].append(
                        VmdMorphFrame(0, part_morph.name, ratio)
                    )
                    morph_matrixes = motion.animate_bone([0], model)

                    for bone_offset in part_morph.offsets:
                        bone_vertices = model.vertices_by_bones.get(
                            bone_offset.bone_index, []
                        )
                        for bone_vertex_index in bone_vertices:
                            vertex = model.vertices[bone_vertex_index]
                            mat = np.zeros((4, 4))
